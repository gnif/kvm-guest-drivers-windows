...
/*
    This structure is for use with the IOCTL_IVSHMEM_REQUEST_MEMALIAS IOCTL

    Please Note:
      - There can only be one memory alias at a time.
      - This feature is dangerous and must be enabled in the qemu configuration first.
      - The target address must reside within the userspace application.
      - The size must be greater then zero and no larger then the shared memory region.
      - The target address must be pinned/fixed in physical ram.
*/
typedef struct IVSHMEM_MEMALIAS
{
    PVOID  target; // the target address to alias over
    UINT32 offset; // start offset of the shared memory region
    UINT32 size;   // the size of the shared memory region to alias
}
IVSHMEM_MEMALIAS, *PIVSHMEM_MEMALIAS;
...
#define IOCTL_IVSHMEM_REQUEST_MEMALIAS CTL_CODE(FILE_DEVICE_UNKNOWN, 0x806, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_IVSHMEM_RELEASE_MEMALIAS CTL_CODE(FILE_DEVICE_UNKNOWN, 0x807, METHOD_BUFFERED, FILE_ANY_ACCESS)
